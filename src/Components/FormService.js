...
export const ControlledInput = ({childConnected, service, setService, dataService, setDataService, setNavReparateur}) => {
  ...
  return (
    <div className="form-recherche">
      <Box fill align="center" justify="center">
        <Box width="medium">
          <Form
          onChange={event => setService({titre, description, tarif, zone})}
            onReset={() => {
              setTitre('');
              setDescription('');
              setTarif('');
            ...
            onSubmit={event =>
                setService(event.value) + fetch('http://localhost/Back-end/AddService.php', {
                    method: 'POST',
                    body: JSON.stringify({
                        childConnected,
                        service
                ...
                .then(data => setDataService(data)) + dataService.successAjout ? <><ToastApprouved /></> + setTimeout(function(){setNavReparateur('Services')},1500) : <Toast />
            }
          >
